...
export class InventoryListQueryDto {
  ...
  @ApiPropertyOptional({ description: 'Lot quality state, such as pending, approved, or quarantined.', example: 'approved' })
  @IsOptional() @IsString() @MaxLength(16) qualityStatus?: string;
  @ApiPropertyOptional({ description: 'When true, return only balances with available stock.', example: true })
  @IsOptional() @Transform(booleanValue) @IsBoolean() inStock?: boolean;
  @ApiPropertyOptional({ description: 'When true, return stock at or below its replenishment minimum.', example: true })
  @IsOptional() @Transform(booleanValue) @IsBoolean() lowStock?: boolean;
  @ApiPropertyOptional({ description: 'When true, return lots expiring within expiryDays.', example: true })
  @IsOptional() @Transform(booleanValue) @IsBoolean() expiringSoon?: boolean;
  @ApiPropertyOptional({ description: 'When true, return lots whose expiry date has passed.', example: false })
  @IsOptional() @Transform(booleanValue) @IsBoolean() expired?: boolean;
  @ApiPropertyOptional({ description: 'Number of days used by expiringSoon.', default: 90, minimum: 1, maximum: 730 })
  @IsOptional() @Type(() => Number) @IsInt() @Min(1) @Max(730) expiryDays = 90;
  @ApiPropertyOptional({ description: 'Search product name, SKU, or batch number.', example: 'paracetamol' })
  @IsOptional() @IsString() @MaxLength(200) search?: string;
  @ApiPropertyOptional({ description: 'Allowed sort key.', enum: ['productName', 'expiresAt', 'availableQty', 'updatedAt'], default: 'productName' })
  ...
  @ApiPropertyOptional({ description: 'Maximum number of records per page.', default: 20, minimum: 1, maximum: 100 })
...
export class InventoryLotCreateDto {
  ...
  @ApiProperty({ description: 'Supplier/manufacturer batch identifier.', example: 'BATCH-2026-001' })
  @IsString() @MinLength(1) @MaxLength(128) batchNumber!: string;
  @ApiPropertyOptional({ format: 'date', description: 'Manufacturing date in ISO date format.', example: '2026-01-15' })
  @IsOptional() @IsDateString() manufacturedAt?: string;
  @ApiProperty({ format: 'date', description: 'Expiry date in ISO date format.', example: '2028-01-15' })
  ...
  @ApiPropertyOptional({ description: 'Quality state assigned to the lot.', default: 'pending', example: 'pending' })
  @IsOptional() @IsString() @MaxLength(16) qualityStatus = 'pending';
  @ApiProperty({ description: 'Recall state assigned to the lot.', default: 'clear', example: 'clear' })
  ...
  @ApiPropertyOptional({ description: 'Initial on-hand quantity as a decimal string.', default: '0', example: '100' })
  @IsOptional() @IsNumberString() initialQuantity = '0';
  @ApiProperty({ description: 'Client idempotency key for safe retry of receipt creation.', example: 'goods-receipt-2026-001' })
...
export class InventoryLotUpdateDto {
  @ApiPropertyOptional({ description: 'Updated purchase cost as a decimal string.', example: '43.00' })
  @IsOptional() @IsNumberString() purchaseCost?: string;
  @ApiPropertyOptional({ description: 'Updated maximum retail price as a decimal string.', example: '60.00' })
  ...
  @IsOptional() @IsString() @MaxLength(32) recallStatus?: string;
...
export class InventoryAdjustmentDto {
  ...
  @IsUUID() lotId!: string;
  @ApiProperty({ description: 'Signed quantity delta. Positive adds stock; negative removes stock.', example: '5.000' })
  @IsNumberString() quantityDelta!: string;
  @ApiProperty({ description: 'Business reason for the adjustment.', example: 'damaged_stock_correction' })
  ...
  @ApiPropertyOptional({ format: 'uuid', description: 'Optional external document or order reference.' })
  @IsOptional() @IsUUID() referenceId?: string;
  @ApiProperty({ description: 'Client idempotency key. Retrying the same key does not post twice.', example: 'adjustment-scan-2026-001' })
  @IsString() @MinLength(8) @MaxLength(128) idempotencyKey!: string;
}
...
export class InventoryReservationDto {
  ...
  @ApiProperty({ format: 'date-time', description: 'Time after which an active reservation can be expired.' })
...
export class ExpireReservationsDto {
  @ApiPropertyOptional({ description: 'Maximum number of expired reservations to process.', default: 100, minimum: 1, maximum: 1000 })
...
export class InventoryHoldDto {
  ...
  @IsUUID() lotId!: string;
  @ApiProperty({ description: 'Quantity to quarantine/hold as a decimal string.', example: '1.000' })
  @IsNumberString() quantity!: string;
...
export class InventoryRelocationDto {
  ...
  @IsOptional() @IsUUID() referenceId?: string;
  @ApiProperty({ description: 'Client idempotency key for safe retry.', example: 'bin-relocation-2026-001' })
  @IsString() @MinLength(8) @MaxLength(128) idempotencyKey!: string;
}
...
export class StockTransferItemDto {
  ...
  @IsUUID() lotId!: string;
  @ApiProperty({ description: 'Quantity requested for transfer as a decimal string.', example: '25.000' })
...
export class StockTransferCreateDto {
  ...
  @ApiProperty({ type: () => [StockTransferItemDto], description: 'Lots and quantities included in the transfer.' })
  @IsArray()
  @ArrayMinSize(1)
  @ArrayMaxSize(500)
...
export class StockTransferReceiveItemDto {
  ...
  @IsUUID() destinationBinId!: string;
  @ApiProperty({ description: 'Quantity physically received as a decimal string.', example: '25.000' })
...
export class StockTransferReceiveDto {
  @ApiProperty({ type: () => [StockTransferReceiveItemDto], description: 'Received quantities grouped by transfer item.' })
  @IsArray()
  @ArrayMinSize(1)
  @ArrayMaxSize(500)
  @ValidateNested({ each: true })
  ...
  @ApiProperty({ description: 'Client idempotency key for safe receive retry.', example: 'transfer-receive-2026-001' })
  @IsString() @MinLength(8) @MaxLength(128) idempotencyKey!: string;
}
...
export class InventoryOperationalListQueryDto {
  ...
  @ApiPropertyOptional({ format: 'uuid', description: 'Filter transfers by destination warehouse.' })
  ...
  @IsOptional() @IsString() @MaxLength(64) reasonCode?: string;
  @ApiPropertyOptional({ description: 'Filter ledger entries by movement type.', example: 'receipt' })
  @IsOptional() @IsString() @MaxLength(64) movementType?: string;
  @ApiPropertyOptional({ description: 'Filter ledger entries by reference type.', example: 'stock_transfer' })
  @IsOptional() @IsString() @MaxLength(64) referenceType?: string;
  @ApiPropertyOptional({ format: 'uuid', description: 'Filter ledger entries by reference UUID.' })
  @IsOptional() @IsUUID() referenceId?: string;
  @ApiPropertyOptional({ format: 'date-time', description: 'Include ledger entries on or after this time.' })
  @IsOptional() @IsDateString() from?: string;
  @ApiPropertyOptional({ format: 'date-time', description: 'Include ledger entries before this time.' })
...
export class CycleCountCreateDto {
  ...
  @IsUUID() warehouseId!: string;
  @ApiPropertyOptional({ format: 'uuid', description: 'Optional bin scope. If omitted, the count covers the warehouse.' })
  ...
  @IsOptional() @IsString() @MaxLength(32) countMode = 'full';
}
...
export class CycleCountItemCreateDto {
  ...
  @IsUUID() cycleCountId!: string;
  @ApiProperty({ format: 'uuid', description: 'Bin being counted.' })
  @IsUUID() binId!: string;
  @ApiProperty({ format: 'uuid', description: 'Lot being counted.' })
  @IsUUID() lotId!: string;
  @ApiPropertyOptional({ description: 'Reason associated with an expected variance.', example: 'manual_review' })
  @IsOptional() @IsString() @MaxLength(64) reasonCode?: string;
}
...
export class CycleCountItemUpdateDto {
  ...
  @ApiProperty({ description: 'Physically counted quantity as a decimal string.', example: '98.000' })
  @IsNumberString() countedQty!: string;
  @ApiPropertyOptional({ description: 'Reason for the counted variance.', example: 'damaged_stock' })
  @IsOptional() @IsString() @MaxLength(64) reasonCode?: string;
}
...
export class CycleCountUpdateDto {
  ...
  @ApiPropertyOptional({ format: 'date-time', description: 'Time counting completed. Completing a count posts recorded variances.' })
