...
export class InventoryService {
  ...
  async execute(operation: string, input: unknown = {}, actor?: string): Promise<unknown> {
    ...
    switch (operation) {
      case 'list-warehouses':
        return this.listWarehouses(values);
      case 'create-warehouse':
        return this.createWarehouse(values, actor);
      case 'get-warehouse':
        return this.getWarehouse(values.warehouseId, values.includeDeleted);
      case 'update-warehouse':
        return this.updateWarehouse(values, actor);
      case 'deactivate-warehouse':
        return this.deactivateWarehouse(values.warehouseId, actor);
      case 'reactivate-warehouse':
        return this.reactivateWarehouse(values.warehouseId, actor);
      case 'list-warehouse-bins':
        return this.listWarehouseBins(values.warehouseId);
      case 'upsert-replenishment-rule':
        return this.upsertReplenishmentRule(values, actor);
      case 'list-replenishment-rules':
        return this.listReplenishmentRules(values);
      case 'list-inventory':
        return this.listInventory(values);
      case 'list-lots':
        return this.listLots(values);
      case 'create-lot':
        return this.createLot(values, actor);
      case 'update-lot':
        return this.updateLot(values.lotId, values, actor);
      case 'get-lot':
        return this.getLot(values.lotId);
      case 'adjust':
        return this.adjust(values, actor);
      case 'list-adjustments':
        return this.listAdjustments(values);
      case 'get-adjustment':
        return this.getAdjustment(values.adjustmentId);
      case 'reserve':
        return this.reserve(values, actor);
      case 'list-reservations':
        return this.listReservations(values);
      case 'get-reservation':
        return this.getReservation(values.reservationId);
      case 'expire-reservations':
        return this.expireReservations(values.limit ?? 100, actor);
      case 'release-reservation':
        return this.changeReservation(values.reservationId, 'released', actor);
      case 'commit-reservation':
        return this.changeReservation(values.reservationId, 'committed', actor);
      case 'create-hold':
        return this.createHold(values, actor);
      case 'list-holds':
        return this.listHolds(values);
      case 'get-hold':
        return this.getHold(values.holdId);
      case 'release-hold':
        return this.releaseHold(values.holdId, actor);
      case 'list-locations':
        return this.listLocations(values.warehouseId);
      case 'relocate':
        return this.relocate(values, actor);
      case 'list-ledger':
        return this.listLedger(values);
      case 'create-transfer':
        return this.createTransfer(values, actor);
      case 'list-transfers':
        return this.listTransfers(values);
      case 'get-transfer':
        return this.getTransfer(values.transferId);
      case 'dispatch-transfer':
        return this.dispatchTransfer(values.transferId, actor);
      case 'receive-transfer':
        return this.receiveTransfer(values, actor);
      case 'product-inventory':
        return this.repository.listProductLocations(values.productId, values.warehouseId);
      case 'product-warehouses':
        return this.repository.listProductWarehouses(values.productId);
      case 'warehouse-inventory':
        return this.page(
          this.repository.listWarehouseInventory(values.warehouseId, inventoryQuery(values)),
          values,
        );
      case 'warehouse-products':
        return this.page(
          this.repository.listWarehouseProducts(values.warehouseId, inventoryQuery(values)),
          values,
        );
      case 'create-cycle-count':
        return this.createCycleCount(values, actor);
      case 'list-cycle-counts':
        return this.listCycleCounts(values);
      case 'get-cycle-count':
        return this.getCycleCount(values.cycleCountId);
      case 'update-cycle-count':
        return this.updateCycleCount(values, actor);
      case 'add-cycle-count-item':
        return this.addCycleCountItem(values, actor);
      case 'count-cycle-count-item':
        return this.countCycleCountItem(values, actor);
      default:
        throw new InventoryValidationError(`Unsupported inventory operation: ${operation}.`);
    }
  }
  ...
  async getWarehouse(id: string, includeDeleted = false) {
    ...
    if (!row)
      throw new InventoryNotFoundError('WAREHOUSE_NOT_FOUND', 'The warehouse was not found.');
    return row;
  }

  // * Function [updateWarehouse]: Applies an optimistic-locked warehouse update.
  async updateWarehouse(input: Row, actor?: string) {
    if (!Number.isInteger(Number(input.rowVersion)))
      throw new InventoryValidationError('rowVersion is required for warehouse updates.');
    const current = await this.repository.getWarehouse(input.warehouseId, false, true);
    if (!current)
      throw new InventoryNotFoundError('WAREHOUSE_NOT_FOUND', 'The warehouse was not found.');
    if (Number(current.rowVersion) !== Number(input.rowVersion))
      throw new InventoryConflictError(
        'CONCURRENT_UPDATE',
        'The warehouse changed after it was loaded.',
        { currentRowVersion: current.rowVersion },
      );
    const values = patch(input, ['warehouseId', 'rowVersion']);
    if (!Object.keys(values).length)
      throw new InventoryValidationError('At least one mutable warehouse field must be provided.');
  ...
  async deactivateWarehouse(id: string, actor?: string) {
    const changed = await this.repository.transaction(() =>
      this.repository.deactivateWarehouse(id, actor ?? null),
    );
    if (!changed)
      throw new InventoryNotFoundError(
        'WAREHOUSE_NOT_FOUND',
        'The warehouse was not found or is already inactive.',
      );
  ...
  async reactivateWarehouse(id: string, actor?: string) {
    const row = await this.repository.transaction(() =>
      this.repository.reactivateWarehouse(id, actor ?? null),
    );
    if (!row)
      throw new InventoryNotFoundError(
        'WAREHOUSE_NOT_FOUND',
        'The deleted warehouse was not found.',
      );
    return row;
  }
  ...
  async upsertReplenishmentRule(input: Row, actor?: string) {
    ...
    if (minimum < 0 || maximum < minimum || reorder <= 0)
      throw new InventoryValidationError(
        'Replenishment quantities must satisfy 0 <= minimumQuantity <= maximumQuantity and reorderQuantity > 0.',
      );
    await this.assertWarehouse(input.warehouseId);
    await this.assertProductAndVariant(input.productId, input.variantId);
    return this.repository.transaction(() =>
      this.repository.upsertReplenishmentRule(input, actor ?? null),
    );
  }
  ...
  async updateLot(id: string, input: Row, actor?: string) {
    if (!Number.isInteger(Number(input.rowVersion)))
      throw new InventoryValidationError('rowVersion is required for lot updates.');
    const row = await this.repository.updateLot(
      id,
      patch(input, ['lotId', 'rowVersion']),
      actor ?? null,
      Number(input.rowVersion),
    );
    if (!row)
      throw new InventoryConflictError(
        'CONCURRENT_UPDATE',
        'The lot changed after it was loaded, or it does not exist.',
      );
    return row;
  }
  ...
  async adjust(input: Row, actor?: string) {
    ...
    return this.repository.transaction(async () => {
      ...
      const balance = await this.repository.getBalanceForUpdate(
        input.warehouseId,
        input.binId,
        input.lotId,
      );
      ...
      if (currentOnHand + quantity < 0)
        throw new InventoryConflictError(
          'INSUFFICIENT_STOCK',
          'The adjustment would make on-hand stock negative.',
        );
      const adjustment = await this.repository.createAdjustment(
        {
          adjustmentNumber: numberId('ADJ'),
          warehouseId: input.warehouseId,
          reasonCode: input.reasonCode,
        },
        userId,
      );
      await this.repository.createAdjustmentItem(
        { ...input, adjustmentId: adjustment.id, quantityDelta: decimal(quantity) },
        userId,
      );
      const next = await this.repository.upsertBalance({
        ...
        binId: input.binId,
        lotId: input.lotId,
        onHandQty: decimal(currentOnHand + quantity),
        reservedQty: balance?.reservedQty ?? '0',
        damagedQty: balance?.damagedQty ?? '0',
        quarantinedQty: balance?.quarantinedQty ?? '0',
        actorUserId: userId,
      });
      await this.repository.createLedger({
        warehouseId: input.warehouseId,
  ...
  async getAdjustment(id: string) {
    ...
    if (!row)
      throw new InventoryNotFoundError(
        ...
        'The inventory adjustment was not found.',
      );
  ...
  async reserve(input: Row, actor?: string) {
    ...
    if (new Date(input.expiresAt).getTime() <= Date.now())
      throw new InventoryValidationError('expiresAt must be in the future.');
    return this.repository.transaction(async () => {
      await this.assertWarehouse(input.warehouseId);
      await this.assertBin(input.binId, input.warehouseId);
      await this.assertLot(input.lotId, input.warehouseId);
      ...
      if (duplicate)
        throw new InventoryConflictError(
          ...
          'The reservation number already exists.',
        );
      ...
      await this.repository.upsertBalance({
        ...balance,
        reservedQty: decimal(number(balance.reservedQty) + quantity),
        actorUserId: actor ?? null,
      });
      await this.repository.createLedger({
        warehouseId: input.warehouseId,
  ...
  async expireReservations(limit: number, actor?: string) {
    const expired = await this.repository.transaction(async () => {
      const candidates = await this.repository.listReservations({
        status: 'active',
      ...
      for (const candidate of candidates.rows)
        results.push(await this.releaseReservationInTransaction(candidate.id, 'expired', actor));
      return results;
    });
    return {
      data: { items: expired },
      pagination: { totalCount: expired.length, limit, offset: 0, hasNext: false },
    };
  }
  ...
  async changeReservation(id: string, status: 'released' | 'committed', actor?: string) {
    return this.repository.transaction(() =>
      this.releaseReservationInTransaction(id, status, actor),
    );
  }
  ...
  private async releaseReservationInTransaction(
    ...
    if (reservation.status !== 'active')
      throw new InventoryConflictError(
        ...
        `An ${reservation.status} reservation cannot be changed.`,
      );
    ...
    if (onHand < 0)
      throw new InventoryConflictError(
        'INSUFFICIENT_STOCK',
        'The reservation cannot be committed because stock is no longer available.',
      );
    const next = await this.repository.upsertBalance({
      ...balance,
      onHandQty: decimal(onHand),
      reservedQty: decimal(number(balance.reservedQty) - quantity),
      actorUserId: actor ?? null,
    });
    ...
    if (!updated)
      throw new InventoryConflictError(
        ...
        'The reservation state could not be updated.',
      );
    await this.repository.createLedger({
      warehouseId: reservation.warehouseId,
      binId: reservation.binId,
      lotId: reservation.lotId,
      movementType: status === 'committed' ? 'reservation_commit' : 'reservation_release',
      quantity: decimal(status === 'committed' ? -quantity : 0),
      referenceType: 'stock_reservation',
      referenceId: id,
      idempotencyKey: `reservation:${id}:${status}`,
      actorUserId: actor ?? null,
    });
  ...
  async createHold(input: Row, actor?: string) {
    ...
    return this.repository.transaction(async () => {
      ...
      const balance = await this.requireBalance(input.warehouseId, input.binId, input.lotId);
      const quantity = number(input.quantity);
      if (availableQuantity(balance) < quantity)
        throw new InventoryConflictError(
          'INSUFFICIENT_STOCK',
          'There is not enough available stock to place the hold.',
        );
      const hold = await this.repository.createHold(input, actor ?? null);
      await this.repository.upsertBalance({
        ...balance,
        quarantinedQty: decimal(number(balance.quarantinedQty) + quantity),
        actorUserId: actor ?? null,
      });
      await this.repository.createLedger({
        warehouseId: input.warehouseId,
  ...
  async getHold(id: string) {
    ...
    if (!row)
      throw new InventoryNotFoundError('HOLD_NOT_FOUND', 'The inventory hold was not found.');
    return row;
  }
  ...
  async releaseHold(id: string, actor?: string) {
    return this.repository.transaction(async () => {
      ...
      if (!released)
        throw new InventoryConflictError('HOLD_UPDATE_FAILED', 'The hold could not be released.');
      await this.repository.createLedger({
        warehouseId: hold.warehouseId,
        binId: hold.binId,
        lotId: hold.lotId,
        movementType: 'hold_release',
        quantity: decimal(0),
        referenceType: 'stock_hold',
        referenceId: id,
        idempotencyKey: `hold:${id}:release`,
        actorUserId: actor ?? null,
      });
  ...
  async relocate(input: Row, actor?: string) {
    ...
    if (input.sourceBinId === input.destinationBinId)
      throw new InventoryValidationError('sourceBinId and destinationBinId must be different.');
    return this.repository.transaction(async () => {
      ...
      if (availableQuantity(source) < quantity)
        throw new InventoryConflictError(
          'INSUFFICIENT_STOCK',
          'There is not enough available stock to relocate.',
        );
      const destination = await this.repository.getBalanceForUpdate(
        input.warehouseId,
      ...
      await this.repository.upsertBalance({
        ...source,
        onHandQty: decimal(number(source.onHandQty) - quantity),
        actorUserId: actor ?? null,
      });
      await this.repository.upsertBalance({
        warehouseId: input.warehouseId,
        ...
        lotId: input.lotId,
        onHandQty: decimal(number(destination?.onHandQty ?? 0) + quantity),
        reservedQty: destination?.reservedQty ?? '0',
        damagedQty: destination?.damagedQty ?? '0',
        quarantinedQty: destination?.quarantinedQty ?? '0',
        actorUserId: actor ?? null,
      });
      await this.repository.createLedger({
        warehouseId: input.warehouseId,
        ...
        lotId: input.lotId,
        movementType: 'relocation_out',
        quantity: decimal(-quantity),
        referenceType: 'stock_relocation',
  ...
  async createTransfer(input: Row, actor?: string) {
    ...
    return this.repository.transaction(async () => {
      ...
      if (await this.repository.findTransferByNumber(input.transferNumber))
        throw new InventoryConflictError(
          'TRANSFER_ALREADY_EXISTS',
          'The transfer number already exists.',
        );
      ...
      for (const item of (input.items ?? []) as Row[]) {
        ...
        items.push(
          await this.repository.createTransferItem({ transferId: transfer.id, ...item }, userId),
        );
      }
  ...
  async getTransfer(id: string) {
    ...
    if (!row)
      throw new InventoryNotFoundError('TRANSFER_NOT_FOUND', 'The stock transfer was not found.');
    ...
    return this.repository.transaction(async () => {
      ...
      if (transfer.status !== 'requested' && transfer.status !== 'approved')
        throw new InventoryConflictError(
          ...
          `A ${transfer.status} transfer cannot be dispatched.`,
        );
      const items = await this.repository.listTransferItems(id, true);
      for (const item of items) {
        let remaining = number(item.requestedQty);
        const balances = await this.repository.listBalancesForLot(
        ...
        for (const balance of balances) {
          ...
          await this.repository.upsertBalance({
            ...balance,
            onHandQty: decimal(number(balance.onHandQty) - dispatched),
            actorUserId: userId,
          });
          await this.repository.createLedger({
            ...
            quantity: decimal(-dispatched),
            referenceType: 'stock_transfer',
            referenceId: id,
            idempotencyKey: `transfer:${id}:out:${item.id}:${balance.binId}`,
            actorUserId: userId,
          });
        ...
        if (remaining > 0)
          throw new InventoryConflictError(
            'INSUFFICIENT_STOCK',
            `There is not enough stock for transfer item ${item.id}.`,
          );
        await this.repository.updateTransferItem(
          item.id,
      ...
      for (const itemInput of (input.items ?? []) as Row[]) {
        ...
        if (!item)
          throw new InventoryNotFoundError(
            ...
            'A transfer item was not found.',
          );
        if (await this.repository.findLedgerByIdempotency(`${input.idempotencyKey}:${item.id}`))
          continue;
        ...
        if (quantity > remaining)
          throw new InventoryValidationError(
            'receivedQuantity cannot exceed the dispatched quantity.',
          );
        await this.assertBin(itemInput.destinationBinId, transfer.destinationWarehouseId);
        const balance = await this.repository.getBalanceForUpdate(
          transfer.destinationWarehouseId,
          itemInput.destinationBinId,
          item.lotId,
        );
        await this.repository.upsertBalance({
          warehouseId: transfer.destinationWarehouseId,
          binId: itemInput.destinationBinId,
          lotId: item.lotId,
          onHandQty: decimal(number(balance?.onHandQty ?? 0) + quantity),
          reservedQty: balance?.reservedQty ?? '0',
          damagedQty: balance?.damagedQty ?? '0',
          quarantinedQty: balance?.quarantinedQty ?? '0',
          actorUserId: userId,
        ...
        await this.repository.updateTransferItem(
          item.id,
          { receivedQty: decimal(number(item.receivedQty) + quantity) },
          userId,
        );
        item.receivedQty = number(item.receivedQty) + quantity;
      }
      const complete = transferItems.every(
        (item) => number(item.receivedQty) >= number(item.dispatchedQty),
      );
      await this.repository.updateTransferStatus(
        input.transferId,
  ...
  async getCycleCount(id: string) {
    ...
    if (!row)
      throw new InventoryNotFoundError('CYCLE_COUNT_NOT_FOUND', 'The cycle count was not found.');
  ...
  async updateCycleCount(input: Row, actor?: string) {
    return this.repository.transaction(async () => {
      ...
      if (current.status === 'completed' && input.status === 'completed')
        throw new InventoryConflictError(
          'CYCLE_COUNT_ALREADY_COMPLETED',
          'The cycle count has already been completed.',
        );
      if (Number(current.rowVersion) !== Number(input.rowVersion))
        throw new InventoryConflictError(
          'CONCURRENT_UPDATE',
          'The cycle count changed after it was loaded.',
          { currentRowVersion: current.rowVersion },
        );
        ...
        for (const item of items) {
          ...
          if (onHand < 0)
            throw new InventoryConflictError(
              ...
              `Cycle-count variance would make stock negative for item ${item.id}.`,
            );
          await this.repository.upsertBalance({
            ...balance,
            onHandQty: decimal(onHand),
          ...
          await this.repository.createLedger({
            ...
            quantity: decimal(variance),
            referenceType: 'cycle_count',
            referenceId: input.cycleCountId,
            idempotencyKey: `cycle-count:${input.cycleCountId}:${item.id}`,
            actorUserId: userId,
          });
        }
      }
      const row = await this.repository.updateCycleCount(
        input.cycleCountId,
        input,
        actor ?? null,
        Number(input.rowVersion),
  ...
  async addCycleCountItem(input: Row, actor?: string) {
    ...
    return this.repository.createCycleCountItem(
      { ...input, systemQty: balance?.onHandQty ?? '0' },
      actor ?? null,
    );
  }
  ...
  async countCycleCountItem(input: Row, actor?: string) {
    ...
    const item = await this.repository
      .listCycleCountItems(input.cycleCountId)
      .then((items) => items.find((candidate) => candidate.id === input.itemId));
    if (!item)
      throw new InventoryNotFoundError(
        'CYCLE_COUNT_ITEM_NOT_FOUND',
        'The cycle-count item was not found.',
      );
    return this.repository.updateCycleCountItem(
      input.itemId,
      {
        countedQty: decimal(counted),
        varianceQty: decimal(counted - number(item.systemQty)),
        reasonCode: input.reasonCode,
      },
      actor ?? null,
    );
  ...
  protected async assertWarehouse(id: string) {
    if (!(await this.repository.warehouseExists(id)))
      throw new InventoryNotFoundError('WAREHOUSE_NOT_FOUND', 'The warehouse was not found.');
  }
  ...
  protected async assertLocation(id: string) {
    if (!(await this.repository.locationExists(id)))
      throw new InventoryNotFoundError(
        'LOCATION_NOT_FOUND',
        'The organization location was not found.',
      );
  }
  ...
  protected async assertProductAndVariant(productId: string, variantId?: string) {
    if (!(await this.repository.productExists(productId)))
      throw new InventoryNotFoundError('PRODUCT_NOT_FOUND', 'The product was not found.');
    if (variantId && !(await this.repository.variantMatches(variantId, productId)))
      throw new InventoryValidationError('The variant does not belong to the product.');
  }
  ...
  protected async assertBin(binId: string, warehouseId: string) {
    if (!(await this.repository.binExistsForWarehouse(binId, warehouseId)))
      throw new InventoryNotFoundError(
        'BIN_NOT_FOUND',
        'The bin was not found in the selected warehouse.',
      );
  }
  ...
  protected async assertLot(lotId: string, warehouseId?: string) {
    ...
    if (warehouseId && lot.warehouseId !== warehouseId)
      throw new InventoryValidationError('The lot does not belong to the selected warehouse.');
    return lot;
  }
  ...
  protected async requireBalance(warehouseId: string, binId: string, lotId: string) {
    ...
    if (!row)
      throw new InventoryNotFoundError(
        'BALANCE_NOT_FOUND',
        'No stock balance exists for the selected bin and lot.',
      );
    return row;
  }
...
function availableQuantity(balance: Row): number {
  return (
    ...
    number(balance.damagedQty) -
    number(balance.quarantinedQty)
  );
}
...
function number(value: unknown): number {
  ...
  if (!Number.isFinite(parsed))
    throw new InventoryValidationError('All quantity values must be valid numbers.');
  return parsed;
}
...
function requiredActor(actor?: string): string {
  if (
    !actor ||
    !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(actor)
  ) {
