 ...
 * Token generation is not a database side effect and is finalized only after transactional writes succeed.
 */
import { Injectable } from '@nestjs/common';
import { createHmac, randomUUID, timingSafeEqual } from 'node:crypto';
import { AuthenticationError } from '@shared/errors';
import type { EncodedToken, TokenClaims, TokenServicePort } from '../../contracts/auth.ports';
...
export class AuthTokenService implements TokenServicePort {
  ...
  private encode(
    ...
    // JWT payloads are signed after all standard and token-specific claims are assembled.
    const now = Math.floor(Date.now() / 1000);
    const payload = {
      sub: subject,
      token_type: type,
